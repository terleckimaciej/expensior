...
def main():
    parser = argparse.ArgumentParser(description="Classify unclassified transactions using rules")
    parser.add_argument(
        '--db',
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Show what would be classified without saving to DB'
    )
    ...
    try:
        print("Loading unclassified transactions...")
        ...
        print(f"Found {len(df_transactions)} unclassified transactions")
        ...
        if args.dry_run:
            print("\n[DRY RUN] Sample of classified transactions:")
            sample = df_classified[df_classified["rule_id"].notna()][
        ...
        if classified_count > 0:
            ...
            conn.commit()
            print(f"✓ Saved {stats['classified']} classifications")
        else:
            print("\nNo transactions were classified.")
